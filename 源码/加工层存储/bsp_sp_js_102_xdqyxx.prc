CREATE OR REPLACE PROCEDURE BSP_SP_JS_102_XDQYXX (IS_DATE    IN VARCHAR2,
  ...
  --    CAEATE BY USER AT 20251127
  --    需求编号：JLBA202509240001关于开发金融基础数据信贷企业基础信息调研表自动化取数的需求 上线日期：2025-11-27，修改人：白杨，提出人：李楠
  ------------------------------------------------------------------------------------------------------
...
VS_STEP := '0.开始插入目标表';
  ...
  SELECT /*+parallel(4)*/
       VS_TEXT     AS DATA_DATE, --1 数据日期  上年末 YYYY-MM-DD 格式
       ...
       ELSE '9122010170255776XN' END AS ORG_CODE,  --2 金融机构代码
       DECODE(B.ID_TYPE, '236', 'A01', '21', 'A02', 'A03') AS CUST_ID_TYPE,   --3 客户证件类型
       B.ID_NO       AS CUST_ID_NO,      --4 客户证件代码
       '2'           AS STATEMT_VERSION, --5 客户会计报表版本   (客户会计报表版本 1：2002版会计报表 2：2007版会计报表 3:其他版本)
       DECODE(REPORT_SUB_TYP, '9', '1', REPORT_SUB_TYP)    AS STATEMT_TYPE, --6 客户会计报表类型    (客户会计报表类型 1:本部报表2:合并报表)
       SUM(DECODE(A.ID_CODE, '9100', A.ID_VAL, 0))         AS BANK_CASH,    --7 货币资金
       SUM(DECODE(A.ID_CODE, '9102', A.ID_VAL, '9103', A.ID_VAL, 0)) AS NOTES_ACCT_REC, --8 应收票据及应收账款
       SUM(DECODE(A.ID_CODE, '9108', A.ID_VAL, 0)) AS INVENTORY,           --9 存货
       SUM(DECODE(A.ID_CODE, '9111', A.ID_VAL, 0)) AS TOTAL_CURR_A,        --10 流动资产合计
       SUM(DECODE(A.ID_CODE, '9117', A.ID_VAL, 0)) AS FIXED_ASSETS,        --11 固定资产
       SUM(DECODE(A.ID_CODE, '9118', A.ID_VAL, 0)) AS CONSTRUCT_IPROGRESS, --12 在建工程
       SUM(DECODE(A.ID_CODE, '9129', A.ID_VAL, 0)) AS TOTAL_NONCURR_A,     --13 非流动资产合计
       SUM(DECODE(A.ID_CODE, '9130', A.ID_VAL, 0)) AS TOTAL_ASSET,         --14 资产总计
       SUM(DECODE(A.ID_CODE, '9131', A.ID_VAL, 0)) AS SHORT_TERM_LOANS,    --15 短期借款
       SUM(DECODE(A.ID_CODE, '9133', A.ID_VAL, '9134', A.ID_VAL, 0)) AS NOTES_ACCT_PAY,  --16 应付票据及应付账款
       SUM(DECODE(A.ID_CODE, '9143', A.ID_VAL, 0)) AS TOTAL_CURR_L,        --17 流动负债合计
       SUM(DECODE(A.ID_CODE, '9144', A.ID_VAL, 0)) AS LONG_TERM_LOANS,     --18 长期借款
       SUM(DECODE(A.ID_CODE, '9151', A.ID_VAL, 0)) AS TOTAL_NONCURR_L,     --19 非流动负债合计
       SUM(DECODE(A.ID_CODE, '9152', A.ID_VAL, 0)) AS TOTAL_LIABILITY,     --20 负债总计
       SUM(DECODE(A.ID_CODE, '9158', A.ID_VAL, 0)) AS TOTAL_EQUITY,        --21所有者权益合计
       SUM(DECODE(A.ID_CODE, '9170', A.ID_VAL, 0)) AS REVENUE_SALES,       --22 营业收入
       SUM(DECODE(A.ID_CODE, '9171', A.ID_VAL, 0)) AS OPERAT_EXPENSE,      --23 营业成本
       SUM(DECODE(A.ID_CODE, '9172', A.ID_VAL, 0)) AS BUSINESS_OTHER_TAX,  --24 营业税金及附加
       SUM(DECODE(A.ID_CODE, '9173', A.ID_VAL, 0)) AS SELLING_EXPENSE,     --25 销售（营业）费用
       SUM(DECODE(A.ID_CODE, '9174', A.ID_VAL, 0)) AS G_A_EXPENSE,         --26 管理费用
       SUM(DECODE(A.ID_CODE, '9175', A.ID_VAL, 0)) AS FINANCE_EXPENSE,     --27 财务费用
       SUM(DECODE(A.ID_CODE, '9180', A.ID_VAL, 0)) AS OPERAT_PROFIT,       --28 营业利润
       SUM(DECODE(A.ID_CODE, '9184', A.ID_VAL, 0)) AS PROFIT_BEFORE_TAX,   --29 利润总额
       SUM(DECODE(A.ID_CODE, '9185', A.ID_VAL, 0)) AS INCOME_TAX,          --30 所得税
       SUM(DECODE(A.ID_CODE, '9186', A.ID_VAL, 0)) AS NET_PROFIT,          --31 净利润
       A.ORG_NUM  AS ORG_NUM,      --32 内部机构号
       SYS_GUID() AS REPORT_ID,    --33 数据id
       VS_NMONTH  AS CJRQ,         --34 采集日期  上年末 YYYYMMDD 格式
       A.ORG_NUM  AS NBJGH,        --35 内部机构号
       '99'       AS BIZ_LINE_ID,  --36 条线
       ''         AS VERIFY_STATUS,--37 校验状态
       ''         AS BSCJRQ,       --38 报送采集日期
       CASE
           ...
           WHEN A.ORG_NUM LIKE '60%' THEN
            '600000'
           ELSE '990000'
       END AS FRNBJGH,      --39 法人内部机构号
       B.CUST_NAM AS CUST_NAME     --40 客户名称
  FROM (SELECT A.*,
