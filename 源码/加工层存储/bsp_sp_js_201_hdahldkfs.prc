CREATE OR REPLACE PROCEDURE BSP_SP_JS_201_HDAHLDKFS(IS_DATE        IN VARCHAR2,
                                                    OI_RETCODE     OUT INTEGER,
                                                    OI_RETCODE_DEC OUT VARCHAR2) AS
  ------------------------------------------------------------------------------------------------------
  -- 程序名
  -- BSP_SP_JS_201_HDAHLDKFS
  -- 用途:生成接口表 PBOCD_JS_201_HDAHLDKFS  互联网贷款发生额信息表
  -- 参数
  --    IS_DATE 输入变量，传入跑批日期
  --    OI_RETCODE 输出变量，用来标识存储过程执行过程中是否出现异常
  --    CAEATE BY ZHOULP AT 20240926_JLBA202408190009_关于在金融基础数据采集平台新增互联网金融数据采集报送的需求
...
BEGIN
  ...
  VS_PROCEDURE_NAME := 'BSP_SP_JS_201_HDAHLDKFS';
  ...
  -------------------------------------------------------------------------
  --新建/清理分区
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDAHLDKFS', OI_RETCODE);
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDBHLDKFS', OI_RETCODE);
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDCHLDKFS', OI_RETCODE);

  --插入互联网贷款发生额信息表数据 改动时需同步修改存量表
  INSERT INTO PBOCD_JS_201_HDAHLDKFS
    (DATA_DATE, --数据日期
     ORG_CODE, --金融机构代码
     ORG_NUM, --内部机构号
  ...
  --本方
    SELECT /*+PARALLEL(4)*/
     ...
     WHERE T.DATA_DATE = IS_DATE
       ...
       AND T.DRAWDOWN_DT BETWEEN
           --(TRUNC(TO_DATE(IS_DATE, 'YYYYMMDD'), 'Q') - 1) AND
           (TRUNC(TO_DATE(IS_DATE, 'YYYYMMDD'), 'MM') - 1) AND --[2025-03-27] [周立鹏] [JLBA202502130004_制度升级2025][李楠] 调整为月跑季报
           TO_DATE(IS_DATE, 'YYYYMMDD') - 1 --互联网贷款数据晚一天下发，上季末数据本季取
...
------[2025-03-27] [周立鹏] [JLBA202502130004_制度升级2025][李楠] 拆分成单位和个人  开始-----------
  --插入互联网单位贷款发生额信息表数据 改动时需同步修改发生表
  INSERT INTO PBOCD_JS_201_HDBHLDKFS
  ...
  --插入互联网个人贷款发生额信息表数据 改动时需同步修改发生表
  INSERT INTO PBOCD_JS_201_HDCHLDKFS
    (DATA_DATE, --数据日期
     ORG_CODE, --金融机构代码
     ORG_NUM, --内部机构号
     INT_LOAN_ID, --互联网贷款统一识别编码
