CREATE OR REPLACE PROCEDURE BSP_SP_JS_201_HDACLSZDK_HIS(IS_DATE        IN VARCHAR2,
                                                    OI_RETCODE     OUT INTEGER,
                                                    OI_RETCODE_DEC OUT VARCHAR2) AS
  ------------------------------------------------------------------------------------------------------
  -- 程序名
  -- BSP_SP_JS_201_HDACLSZDK
  -- 用途:生成接口表 PBOCD_JS_201_HDACLSZDK  存量数字贷款信息表  与G0107核对
  -- 参数
  --    IS_DATE 输入变量，传入跑批日期
  --    OI_RETCODE 输出变量，用来标识存储过程执行过程中是否出现异常
  --    需求编号：JLBA202502130004_制度升级2025 上线日期：2025-03-27，修改人：周立鹏，提出人：李楠   修改原因：新建
  --    需求编号：配合报送，无需求 上线日期：2025-04-27，修改人：周立鹏，提出人：无   修改原因：取最新日期的标志
  ------------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  VS_PROCEDURE_NAME := 'BSP_SP_JS_201_HDACLSZDK';
  ...
  -------------------------------------------------------------------------
  --新建/清理分区
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDACLSZDK', OI_RETCODE);
  -------------------------------------------------------------------------

  EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK_TMP1';

  --将数字贷款插入到临时表 改动时需同步修改发生表
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK_TMP1
    SELECT A.LOAN_NUM,
           A.ACCT_NUM,
           A.DRAFT_RNG,
           A.ACCT_TYP,
     ...
     WHERE A.FUND_USE_LOC_CD = 'I'
       ...
       /*AND EXISTS (
       SELECT 1 FROM SMTMODS.L_ACCT_LOAN X WHERE X.DATA_DATE='20250327' AND X.LOAN_NUM=A.LOAN_NUM
       AND SUBSTR(X.DIGITAL_ECONOMY_INDUSTRY, 1, 2) IN ('01', '02', '03', '04')
       )*/
    ...
    SELECT A.LOAN_NUM,
     ...
     INNER JOIN SMTMODS.L_CUST_C C
       ...
       AND C.DATA_DATE = IS_DATE
       AND C.CUST_TYP <> '3' --去除个体工商户
     /*INNER JOIN SMTMODS.L_AGRE_BILL_CONTRACT D
        ON D.DATA_DATE = IS_DATE
       AND A.ACCT_NUM = D.BILL_NUM
       AND SUBSTR(D.DIGITAL_ECONOMY_INDUSTRY, 1, 2) IN
           ('01', '02', '03', '04')*/
     WHERE FUND_USE_LOC_CD = 'I'
       ...
       AND A.CANCEL_FLG = 'N'
       AND (A.ITEM_CD LIKE '130101%' OR ITEM_CD LIKE '130104%') --直贴
       AND A.LOAN_STOCKEN_DATE IS NULL --add by haorui 20250228 JLBA202408200012 资产未转让
       AND A.LOAN_ACCT_BAL > 0
       ...
       AND EXISTS (
       --[2025-04-27] [周立鹏] [配合报送，无需求][无] 取最新日期的标志
       SELECT 1 FROM SMTMODS.L_ACCT_LOAN X --WHERE X.DATA_DATE='20250406' 
       WHERE X.DATA_DATE = to_char(sysdate-1,'yyyymmdd')
       AND X.LOAN_NUM=A.LOAN_NUM
       AND SUBSTR(X.DIGITAL_ECONOMY_INDUSTRY, 1, 2) IN ('01', '02', '03', '04')
       )
       ;
  COMMIT;
  ...
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK
    (DATA_DATE, -- 数据日期
     FIELD_TYPE, -- 字段类别
     BALANCE_SUM, -- 贷款汇总金额
      ...
      FROM (SELECT 'T01' AS FIELD_TYPE, --字段类别 --数字经济核心产业贷款汇总
                   ...
                   CASE
                     ...
                     ELSE
                      '99'
                   END AS NBJGH --内部机构号
              FROM PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK_TMP1 A
  ...
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK
    ...
    SELECT /*+PARALLEL(4)*/
     ...
     T.DISCOUNT_INTEREST --贴现利息
      FROM PBOCD_DATACORE.PBOCD_JS_201_HDACLSZDK_TMP1 T
