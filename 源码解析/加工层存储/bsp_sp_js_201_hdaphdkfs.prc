CREATE OR REPLACE PROCEDURE BSP_SP_JS_201_HDAPHDKFS(IS_DATE        IN VARCHAR2,
                                                    OI_RETCODE     OUT INTEGER,
                                                    OI_RETCODE_DEC OUT VARCHAR2) AS  ------------------------------------------------------------------------------------------------------
  -- 程序名
  -- BSP_SP_JS_201_HDAPHDKFS
  -- 业务域: 贷款类
  -- 用途: 生成接口表 PBOCD_JS_201_HDAPHDKFS  存量普惠贷款信息  --逻辑拷贝自大集中A3302
  -- 输出接口表: PBOCD_JS_201_HDAPHDKFS
  -- 参数
  --    IS_DATE 输入，传入跑批日期
  --    OI_RETCODE 输出，用来标识存储过程执行过程中是否出现异常
  -- 引用的监管集市表
  --    SMTMODS.AGRE_CREDITLINE_INFO                       — AGRE_CREDITLINE_INFO
  --    SMTMODS.L_ACCT_LOAN                                — 贷款借据信息表
  --    SMTMODS.L_ACCT_TRAD_FIN                            — 贸易融资补充信息
  --    SMTMODS.L_CUST_ALL                                 — 全量客户信息表
  --    SMTMODS.L_CUST_C                                   — 对公客户补充信息表
  --    SMTMODS.L_CUST_P                                   — 对私客户补充信息表
  --    SMTMODS.L_PUBL_RATE                                — 汇率表
  --    SMTMODS.V_PUB_IDX_DK_GRSNDK                        — V_PUB_IDX_DK_GRSNDK
  --    SMTMODS.V_PUB_IDX_DK_GTGSHSNDK                     — V_PUB_IDX_DK_GTGSHSNDK
  -- 修改历史
...
BEGIN
  ...
  VS_PROCEDURE_NAME := 'BSP_SP_JS_201_HDAPHDKFS';
  ...
  -------------------------------------------------------------------------
  --新建/清理分区
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDAPHDKFS', OI_RETCODE);

  EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.A_REPT_ITEM_VAL_A3302_TMP3'; --个体工商户和小微企业主客户中间表
  EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1'; --各项贷款标识临时表
  -------------------------------------------------------------------------


EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN';

INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN
  (data_date,
   acct_num,
   loan_num,
   book_type,
  ...
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1
    (ACCT_NUM, LOAN_NUM, FLAG)
    SELECT /*+ parallel(4)*/
     A.ACCT_NUM, A.LOAN_NUM, '1XW' AS FLAG
      FROM PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN A
  ...
  --单户授信小于500万元的农户经营性贷款  参照大集中A3302
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1
    (ACCT_NUM, LOAN_NUM, FLAG)
    SELECT /*+ parallel(4)*/
     ...
     FROM (
       ...
       INNER JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN B --贷款借据信息表
       ...
       WHERE B.ACCT_TYP NOT LIKE '90%' --去除委托贷款
         AND TO_CHAR(B.DRAWDOWN_DT, 'YYYYMM') = SUBSTR(IS_DATE, 1, 6)
 ...
 INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS  
(DATA_DATE,
FIELD_TYPE,
BALANCE_SUM,
INT_RATE_WA,
...
INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS
   ...
   NBJGH || '0000' --法人内部机构号
    FROM (SELECT /*+ parallel(4)*/
           ...
           END AS NBJGH --内部机构号
            ...
            FROM PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN A -- 贷款借据信息表
            LEFT JOIN SMTMODS.L_PUBL_RATE U --汇率表
              ON U.DATA_DATE = IS_DATE
             AND U.CCY_DATE = TO_DATE(IS_DATE, 'YYYYMMDD')
             AND U.BASIC_CCY = A.CURR_CD --基准币种
 ...
 INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS
    ...
    SELECT /*+PARALLEL(4)*/
     ...
     'T99'AS FIELD_TYPE
      FROM PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN T -- 贷款借据信息表
      LEFT JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1 XW --单户授信小于1000万元的小微型企业贷款
        ON XW.LOAN_NUM = T.LOAN_NUM
       AND XW.FLAG = '1XW'
      LEFT JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1 GTJY --单户授信小于1000万元的个体工商户经营性贷款
        ON GTJY.LOAN_NUM = T.LOAN_NUM
       AND GTJY.FLAG = '2GTJY'
      LEFT JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1 XWJY --单户授信小于1000万元的小微企业主经营性贷款
        ON XWJY.LOAN_NUM = T.LOAN_NUM
       AND XWJY.FLAG = '3XWJY'
      LEFT JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_TMP1 NHJY --单户授信小于500万元的农户经营性贷款
        ON NHJY.LOAN_NUM = T.LOAN_NUM
       AND NHJY.FLAG = '4NHJY'
      LEFT JOIN PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN CYDB --创业担保贷款  参照金数专项贷款
  ...
  AND NOT EXISTS(-- 剔除当天发生当天收回的垫款
         SELECT 1 FROM PBOCD_DATACORE.PBOCD_JS_201_HDAPHDKFS_LOAN A2
