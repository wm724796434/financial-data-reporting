CREATE OR REPLACE PROCEDURE BSP_SP_JS_201_HDAKJDKFS(IS_DATE        IN VARCHAR2,
                                                    OI_RETCODE     OUT INTEGER,
                                                    OI_RETCODE_DEC OUT VARCHAR2) AS  ------------------------------------------------------------------------------------------------------
  -- 程序名
  -- BSP_SP_JS_201_HDAKJDKFS
  -- 业务域: 贷款类
  -- 用途: 生成接口表 PBOCD_JS_201_HDAKJDKFS  存量科技贷款信息表
  -- 输出接口表: PBOCD_JS_201_HDAKJDKFS
  -- 参数
  --    IS_DATE 输入，传入跑批日期
  --    OI_RETCODE 输出，用来标识存储过程执行过程中是否出现异常
  -- 引用的监管集市表
  --    SMTMODS.INTO_FIELD_INDEX                           — INTO_FIELD_INDEX
...
BEGIN
  ...
  -------------------------------------------------------------------------
  --新建/清理分区
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDAKJDKFS', OI_RETCODE);

EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS_LOAN';

INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS_LOAN
  (data_date,
   acct_num,
   loan_num,
   book_type,
  ...
  --插入T01-T12汇总报送数据 改动时需同步修改存量表
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS
    (DATA_DATE, --数据日期
     FIELD_TYPE, --字段类别
     BALANCE_SUM, --贷款汇总金额
    ...
    SELECT /*+PARALLEL(4)*/
            ...
            SELECT 'T08' AS FIELD_TYPE, --字段类别
                    ...
                    CASE
                      ...
                      ELSE
                       '99'
                    END AS NBJGH --内部机构号
              FROM PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS_LOAN T --贷款借据信息表
             ...
             WHERE T.DATA_DATE = IS_DATE --取数逻辑参考1104_S7001和一表通T_2_1
               ...
               AND (--完全复制S72逻辑 312-403行
                 ...
                 --AND nvl(C.HIGH_TECH_MNFT,'0') <>'0'  -- ALTER BY SHIYU 20241029
     ...
     AND NOT EXISTS(-- 剔除当天发生当天收回的垫款
         SELECT 1 FROM PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS_LOAN A2
     ...
     AND T.LOAN_STOCKEN_DATE IS NULL*/    --add by haorui 20250311 JLBA202408200012 资产未转让
         ...
         AND nvl(C.PANT_DENS_INDU,'0')<>'0'--ALTER BY SHIYU 20241029
          )
         OR
         (T.FUND_USE_LOC_CD = 'I'
         AND T.CANCEL_FLG = 'N'
...
AND NOT EXISTS(-- 剔除当天发生当天收回的垫款
         SELECT 1 FROM PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS_LOAN A2
            ...
            SELECT 'T12' AS FIELD_TYPE, --字段类别
             ...
             AND TO_CHAR(T.DRAWDOWN_DT, 'YYYYMM') = SUBSTR(IS_DATE, 1, 6)
               ...
               /*TO_CHAR(T.DRAWDOWN_DT, 'YYYYMM') = SUBSTR(IS_DATE, 1, 6)
               AND T.ACCT_STS <> '3'
               AND T.CANCEL_FLG <> 'Y'
         AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
               AND*/ (T.ACCT_TYP NOT LIKE '01%' OR T.ACCT_TYP LIKE '0102%')
  ...
  INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS
    ...
    SELECT /*+PARALLEL(4)*/
     ...
     CASE
      ...
      AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
       ...
       when t.ITEM_CD not like '130102%' --以摊余成本计量的转贴现
            ...
            and t.CANCEL_FLG <> 'Y'
      AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
            AND (C.HIGH_TECH_MNFT = 'C03' or substr(t.high_tech_mnft,1,2)='03') then 'HTP03'--3.电子及通信设备制造业
     when t.ITEM_CD not like '130102%' --以摊余成本计量的转贴现
            and t.ITEM_CD not like '130105%' --以公允价值计量变动计入权益的转贴现
            AND t.ACCT_TYP NOT IN ('C01', 'D01', 'E01', 'E02')
            AND T.FUND_USE_LOC_CD = 'I'
            ...
            and t.CANCEL_FLG <> 'Y'
      AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
            AND (C.HIGH_TECH_MNFT ='C04' or substr(t.high_tech_mnft,1,2)='04') then 'HTP04'--4.计算机及办公设备制造业
     when t.ITEM_CD not like '130102%' --以摊余成本计量的转贴现
            and t.ITEM_CD not like '130105%' --以公允价值计量变动计入权益的转贴现
            AND t.ACCT_TYP NOT IN ('C01', 'D01', 'E01', 'E02')
            AND T.FUND_USE_LOC_CD = 'I'
            ...
            and t.CANCEL_FLG <> 'Y'
      AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
            AND (C.HIGH_TECH_MNFT ='C05' or substr(t.high_tech_mnft,1,2)='05') then 'HTP05' --5.医疗仪器设备及仪器仪表制造业
     when t.ITEM_CD not like '130102%' --以摊余成本计量的转贴现
            and t.ITEM_CD not like '130105%' --以公允价值计量变动计入权益的转贴现
            AND t.ACCT_TYP NOT IN ('C01', 'D01', 'E01', 'E02')
            AND T.FUND_USE_LOC_CD = 'I'
     ...
     --[2025-03-27] [周立鹏] [JLBA202502130004_制度升级2025][李楠] 新增交易流水号、业务类别字段
     '1', --交易流水号
     CASE
       WHEN B.BILL_NUM IS NULL THEN
        'C01' --单位贷款
     ...
     INNER JOIN SMTMODS.L_CUST_C A --对公客户信息表
      ...
      LEFT JOIN SMTMODS.PUB_KJDK M2
        ON SUBSTR(M2.HYDL, 1, 3) = 'HTS'  --HTS-高技术服务业
       AND/* T.LOAN_PURPOSE_CD*/ 
       (CASE WHEN NVL(C.HIGH_TECH_SRVE,'0') <>'0' AND T.ITEM_CD NOT  LIKE '1301%' THEN C.HIGH_TECH_SRVE
                 WHEN SUBSTR(T.HIGH_TECH_SRVE,1,1) IN ('1','2','3','4','5','6','7','8') AND T.ITEM_CD LIKE '1301%' THEN SUBSTR(T.HIGH_TECH_SRVE,1,1) END
            ) = M2.CODE
      LEFT JOIN SMTMODS.PUB_KJDK M3
        ON SUBSTR(M3.HYDL, 1, 2) = 'PA'   --PA-知识产权（专利）密集型产业
       AND  
       (CASE WHEN nvl(C.PANT_DENS_INDU,'0') <>'0' AND T.ITEM_CD NOT  LIKE '1301%' THEN C.PANT_DENS_INDU
     ...
     AND TO_CHAR(T.DRAWDOWN_DT, 'YYYYMM') = SUBSTR(IS_DATE, 1, 6)
       ...
       (--T.LOAN_ACCT_BAL > 0 --贷款余额大于0
         ...
         AND T.LOAN_STOCKEN_DATE IS NULL    --add by haorui 20250311 JLBA202408200012 资产未转让
               ...
               AND 
                 --[2025-12-11] [周立鹏] [JLBA202510150005_关于票据系统的票交所票据标识功能改造的需求][李楠] 加入票据 高技术制造业、高技术服务业、知识产权 取数逻辑
                 --nvl(C.HIGH_TECH_SRVE,'0')<>'0'  --ALTER BY SHIYU 20241029
...
/*  --以下类别暂缓报送
  UPDATE PBOCD_DATACORE.PBOCD_JS_201_HDAKJDKFS
