CREATE OR REPLACE PROCEDURE BSP_SP_JS_201_HDALSDKFS(IS_DATE        IN VARCHAR2,
                                                    OI_RETCODE     OUT INTEGER,
                                                    OI_RETCODE_DEC OUT VARCHAR2) AS  ------------------------------------------------------------------------------------------------------
  -- 程序名
  -- BSP_SP_JS_201_HDALSDKFS
  -- 业务域: 贷款类
  -- 用途: 生成接口表 PBOCD_JS_201_HDALSDKFS  绿色贷款发生额信息
  -- 输出接口表: PBOCD_JS_201_HDALSDKFS
  -- 参数
  --    IS_DATE 输入，传入跑批日期
...
BEGIN
  ...
  VS_PROCEDURE_NAME := 'BSP_SP_JS_201_HDALSDKFS';
  ...
  -------------------------------------------------------------------------
  --新建/清理分区
  SP_PBOCD_PARTITIONS(IS_DATE, 'PBOCD_JS_201_HDALSDKFS', OI_RETCODE);
  -------------------------------------------------------------------------
  
EXECUTE IMMEDIATE 'TRUNCATE TABLE PBOCD_DATACORE.PBOCD_JS_201_HDALSDKFS_LOAN';

INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDALSDKFS_LOAN
  (data_date,
   acct_num,
   loan_num,
   book_type,
  ...
  -- 20260130 制度升级
 INSERT INTO PBOCD_DATACORE.PBOCD_JS_201_HDALSDKFS  
(DATA_DATE,
FIELD_TYPE,
BALANCE_SUM,
INT_RATE_WA,
...
FRNBJGH
 ...
 FROM (  SELECT CASE  WHEN A2.INLANDORRSHORE_FLG = 'N' AND T.GREEN_CREDIT_USAGE IS NOT NULL THEN 'TW00'
                ...
                CASE WHEN SUBSTR(T.ORG_NUM, 1, 2) BETWEEN '51' AND '60' THEN SUBSTR(T.ORG_NUM, 1, 2)
                     ELSE  '99'
                     END AS NBJGH --内部机构号
      FROM PBOCD_DATACORE.PBOCD_JS_201_HDALSDKFS_LOAN T -- 贷款借据信息表 
     ...
     WHERE T.DATA_DATE = IS_DATE
       ...
       AND NOT EXISTS(-- 剔除当天发生当天收回的垫款
                    SELECT 1 FROM PBOCD_DATACORE.PBOCD_JS_201_HDALSDKFS_LOAN A2
